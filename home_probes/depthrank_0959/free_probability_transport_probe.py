...
# ---------------------------------------------------------------------------
# UNIFIED FRAME-1 x FRAME-5 PROBE — the depth-link
# ---------------------------------------------------------------------------
# Two corp findings looked separate: (FREEPROB) the running PRODUCT effective rank
# over-collapses vs free, monotonically with depth; (DEPTHCONSV) a PER-LAYER
# spectral quantity (spectral_entropy = per-layer eff-rank) is depth-conserved.
# This probe tests that they are ONE mechanism: if each layer keeps ~k_eff
# dimensions (conserved) but consecutive layers route COHERENTLY (positive
# hand-off), the running-product rank decays GEOMETRICALLY in depth (log r_j
# linear in j) at an excess rate over free set by the hand-off. So:
#   conserved per-layer rank + positive hand-off  =>  geometric product over-collapse.
def _eff_rank_trajectory(factor_mats):
    """Running-product effective rank r[j] = eff_rank(prod of first j factors),
    j = 1..L, in compositional order (factor_mats[0] applied first; matches
    _fit_factors_true's P = T @ P)."""
    traj, P = [], None
    for T in factor_mats:
        P = T if P is None else T @ P
        traj.append(float(effective_rank(np.linalg.svd(P, compute_uv=False))))
    return traj


def _free_eff_rank_trajectory(factor_svals, rng, n_draws=12):
    """Free-composition running-product eff-rank trajectory: SAME per-layer
    spectra, Haar rotations between factors (the independent-bases baseline)."""
    L = len(factor_svals)
    acc = np.zeros(L)
    for _ in range(n_draws):
        P = None
        for j, s in enumerate(factor_svals):
            G = _haar_orth(len(s), rng) @ np.diag(s)
            P = G if P is None else G @ P
            acc[j] += float(effective_rank(np.linalg.svd(P, compute_uv=False)))
    return (acc / n_draws).tolist()


def _loglin_slope(y):
    """Fit log(y) ~ a + lam*j over j=0..L-1; return (lam, r2). lam<0 = geometric
    decay; r2 = how GEOMETRIC the decay is (linear-in-log fit quality)."""
    y = np.asarray(y, float)
    m = y > 1e-9
    if int(m.sum()) < 3:
        return float("nan"), float("nan")
    j = np.arange(len(y))[m].astype(float)
    ly = np.log(y[m])
    A = np.vstack([np.ones_like(j), j]).T
    coef, *_ = np.linalg.lstsq(A, ly, rcond=None)
    pred = A @ coef
    ss_res = float(np.sum((ly - pred) ** 2))
    ss_tot = float(np.sum((ly - ly.mean()) ** 2))
    return float(coef[1]), float(1.0 - ss_res / (ss_tot + 1e-12))


def _per_layer_entropy_conservation(factor_mats):
    """DEPTHCONSV spectral_entropy = per-layer eff-rank; conservation = low CV
    across the informative second-half window (depth_conservation_probe spirit)."""
    k = np.array([float(effective_rank(np.linalg.svd(T, compute_uv=False)))
                  for T in factor_mats])
    half = k[len(k) // 2:] if len(k) >= 4 else k
    cv = float(np.std(half) / (np.abs(np.mean(half)) + 1e-12))
    return {"per_layer_eff_rank_mean": round(float(np.mean(k)), 3),
            "per_layer_eff_rank_cv_secondhalf": round(cv, 4),
            "per_layer_eff_rank": [round(float(x), 3) for x in k]}


def _full_product_svals(factor_mats):
    P = None
    for T in factor_mats:
        P = T if P is None else T @ P
    return np.linalg.svd(P, compute_uv=False)
...
def _depth_cell_metrics(factor_mats, true_svals, rng):
    """The unified per-cell measurement. over_collapse_logratio = log(free/true)
    endpoint rank ratio (>0: product over-collapses below free); gap_slope = how
    fast the log(free)-log(true) rank gap GROWS with depth (accumulation rate);
    r2_true_geometric = is the true decay log-linear (geometric flow) at all;
    plus the conserved per-layer rank and the coherent hand-off."""
    r_true = _eff_rank_trajectory(factor_mats)
    r_free = _free_eff_rank_trajectory([np.linalg.svd(T, compute_uv=False)
                                        for T in factor_mats], rng)
    lam_true, r2_true = _loglin_slope(r_true)
    rp = rank_preservation_diag(factor_mats, true_svals, rng)
    over = float(np.log((rp["free_eff_rank"] + 1e-9) / (rp["true_eff_rank"] + 1e-9)))
    gap = np.log(np.asarray(r_free) + 1e-9) - np.log(np.asarray(r_true) + 1e-9)
    gap_slope, _ = _loglin_slope(np.exp(gap)) if np.all(np.exp(gap) > 0) else (float("nan"), 0)
    cons = _per_layer_entropy_conservation(factor_mats)
    return {"r_true_trajectory": [round(x, 3) for x in r_true],
            "r_free_trajectory": [round(x, 3) for x in r_free],
            "over_collapse_logratio": round(over, 4),
            "gap_slope": round(float(gap_slope), 4),
            "lambda_true": round(lam_true, 4),
            "r2_true_geometric": round(r2_true, 3),
            "handoff_minus_baseline": rp["handoff_minus_baseline"],
            "rank_preservation_ratio": rp["rank_preservation_ratio"], **cons}


def _depth_link_synthetic(rng, r=24, L=16):
    """Gate the instrument with three planted regimes. FREE (independent Haar
    bases): over_collapse ~ 0, hand-off ~ 0. COHERENT (top-k input basis aligned
    to previous output basis): over_collapse > 0 with positive hand-off (true
    over-collapses below free). ANTI (top-k aligned to previous WEAK directions):
    over_collapse < 0 with negative hand-off (rank preserved) — checks the metric
    is correctly SIGNED. The geometric R^2 is reported, not gated (the synthetic
    already reveals over-collapse is NOT log-linear)."""
    s = np.sort(np.abs(rng.standard_normal(r)) + 0.2)[::-1]

    def build(mode, k=4):
        mats, Uprev = [], None
        for _ in range(L):
            U = _haar_orth(r, rng)
            if Uprev is None or mode == "free":
                V = _haar_orth(r, rng)
            else:
                G = rng.standard_normal((r, r))
                G[:, :k] = Uprev[:, :k] if mode == "coherent" else Uprev[:, -k:]
                V, _ = np.linalg.qr(G)
            mats.append(U @ np.diag(s) @ V.T)
            Uprev = U
        return mats

    def _cell(m):
        mats = build(m)
        return _depth_cell_metrics(mats, _full_product_svals(mats), rng)
    cells = {m: _cell(m) for m in ("free", "coherent", "anti")}
    f, c, a = cells["free"], cells["coherent"], cells["anti"]
    checks = [
        {"test": "FREE: |over_collapse| < 0.15", "pass": abs(f["over_collapse_logratio"]) < 0.15},
        {"test": "FREE: hand-off < 0.10", "pass": f["handoff_minus_baseline"] < 0.10},
        {"test": "COHERENT: over_collapse > 0.15 (true below free)", "pass": c["over_collapse_logratio"] > 0.15},
        {"test": "COHERENT: hand-off > FREE hand-off", "pass": c["handoff_minus_baseline"] > f["handoff_minus_baseline"]},
        {"test": "ANTI: over_collapse < 0 AND hand-off < 0 (signed)", "pass": a["over_collapse_logratio"] < 0 and a["handoff_minus_baseline"] < 0},
    ]
    npass = sum(ck["pass"] for ck in checks)
    return {"free": {k: f[k] for k in ("over_collapse_logratio", "handoff_minus_baseline", "r2_true_geometric")},
            "coherent": {k: c[k] for k in ("over_collapse_logratio", "handoff_minus_baseline", "r2_true_geometric")},
            "anti": {k: a[k] for k in ("over_collapse_logratio", "handoff_minus_baseline", "r2_true_geometric")},
            "checks": checks, "pass_rate": f"{npass}/{len(checks)}"}


def run_depth_link(models=("gpt2",), n_samples=DEFAULT_N, r=DEFAULT_R, seed=0):
    """UNIFIED FRAME-1 (free/rank) x FRAME-5 (depth conservation) measurement."""
    rng = np.random.default_rng(seed)
    out = {"mode": "depth_link_freeprob_depthconsv",
           "claim": "conserved per-layer eff-rank + positive hand-off => GEOMETRIC "
                    "product-rank over-collapse vs free; lambda_excess tracks hand-off",
           "date": time.strftime("%Y-%m-%d"), "n_samples": n_samples, "r": r,
           "synthetic": _depth_link_synthetic(rng), "results": {}}
    print(f"synthetic gate: {out['synthetic']['pass_rate']}")
    cells = []
    for model in models:
        meta, captured = _fit_factors_true(model, n_samples, r, seed)
        for domain, (fsv, true_svals, _n, mats) in captured.items():
            rec = _depth_cell_metrics(mats, true_svals, rng)
            rec["n_layers"] = meta["n_layers"]
            rec["L_factors"] = len(mats)
            key = f"{model}|{domain}"
            out["results"][key] = rec
            cells.append((meta["n_layers"], rec["handoff_minus_baseline"],
                          rec["over_collapse_logratio"],
                          rec["per_layer_eff_rank_cv_secondhalf"],
                          rec["r2_true_geometric"]))
            print(f"{model:>12} L{meta['n_layers']:>2} {domain:>22} "
                  f"over_collapse={rec['over_collapse_logratio']:+.3f} "
                  f"handoff={rec['handoff_minus_baseline']:+.3f} "
                  f"perlayer_cv={rec['per_layer_eff_rank_cv_secondhalf']:.3f} "
                  f"R2_geom={rec['r2_true_geometric']:.2f}", flush=True)
    if len(cells) >= 3:
        ...
        out["aggregate"] = {
            "n_cells": len(cells),
            "corr_handoff_vs_over_collapse": _corr(1, 2),
            "corr_depth_vs_over_collapse": _corr(0, 2),
            "corr_depth_vs_per_layer_cv": _corr(0, 3),
            "mean_r2_true_geometric": round(float(np.mean(arr[:, 4])), 3),
            "mean_over_collapse": round(float(np.mean(arr[:, 2])), 4),
            "frac_over_collapse_positive": round(float(np.mean(arr[:, 2] > 0)), 3),
        }
        ag = out["aggregate"]
        print(f"  AGG: corr(handoff,over_collapse)={ag['corr_handoff_vs_over_collapse']} "
              f"corr(depth,over_collapse)={ag['corr_depth_vs_over_collapse']} "
              f"mean_over_collapse={ag['mean_over_collapse']} "
              f"frac>0={ag['frac_over_collapse_positive']}")
    path = HERE / "free_probability_transport_depth_link.json"
    path.write_text(json.dumps(out, indent=2))
    print(f"saved -> {path.name}")
    return out
...
def aggregate():
    ...
    skip_stems = ("summary", "synthetic", "diagnostics", "stransform",
                  "alignment", "routing_model", "rank_preservation", "depth_link")
...
def main():
    ...
    ap.add_argument("--depth-link", action="store_true",
                    help="UNIFIED Frame-1 x Frame-5: does the running-product rank collapse GEOMETRICALLY in depth at an excess rate set by the (conserved-per-layer) hand-off?")
    ...
    if args.depth_link:
        models = args.models if args.models else ["gpt2"]
        run_depth_link(models=models, n_samples=args.n_samples,
                       r=args.r, seed=args.seed)
        return
    if args.aggregate:
        aggregate()
