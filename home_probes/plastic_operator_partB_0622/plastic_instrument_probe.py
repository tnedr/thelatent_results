...
def _append_panel_row(panel_path: Path | None, row: dict) -> None:
    if panel_path is None:
        return
    panel_path.parent.mkdir(parents=True, exist_ok=True)
    with panel_path.open("a", encoding="utf-8") as fh:
        fh.write(json.dumps(row, ensure_ascii=False) + "\n")


def _make_prompt(task_family: str, tok, rng, block_len: int, *, n_blocks: int = 1):
    from routing_selection_probe import build_induction, build_recall, _vocab_lohi

    if task_family == "recall":
        n_pairs = max(2, block_len // 2)
        return build_recall(tok, rng, n_pairs, distractors=0)
    if task_family == "factual":
        from factual_recall_tasks import FACTUAL_DOMAINS, make_factual_prompt

        domain = FACTUAL_DOMAINS[int(rng.integers(0, len(FACTUAL_DOMAINS)))]
        return make_factual_prompt(domain, tok, rng, n_shots=4)
    if task_family == "multi_induction" and n_blocks > 1:
        lo, hi = _vocab_lohi(tok)
        block = [int(rng.integers(lo, hi)) for _ in range(block_len)]
        seq = block * n_blocks
        input_ids = seq[:-1]
        return input_ids, len(input_ids) - 1, seq[-1]
    return build_induction(tok, rng, block_len)
...
def _random_baseline_stats(apply, apply_t, g, h_query, ctx, layers, l_c, ids, pos,
                           target, eps_fracs, d, *, random_n, seed):
    if random_n <= 0:
        return {}
    rng = np.random.default_rng(seed)
    tg, cg = [], []
    for _ in range(random_n):
        v_r = rng.standard_normal(d)
        v_r = v_r / (np.linalg.norm(v_r) + 1e-30)
        Tv_r = apply(v_r.reshape(-1, 1))[:, 0]
        tg.append(abs(float(g @ Tv_r)))
        _, _, max_r = _causal_curve(
            ctx, layers, l_c, ids, pos, target, h_query, v_r, eps_fracs)
        cg.append(max_r)
    return {
        "task_gain_random_mean": round(float(np.mean(tg)), 6),
        "task_gain_random_std": round(float(np.std(tg)), 6),
        "causal_random_mean": round(float(np.mean(cg)), 6),
        "causal_random_std": round(float(np.std(cg)), 6),
    }


def _analyze_prompt(ctx, layers, l_c, ids, pos, target, *, iters, alpha, v0,
                    eps_fracs, random_n=0, prompt_seed=0):
    ...
    if random_n > 0:
        rb = _random_baseline_stats(
            apply, apply_t, g, h_query, ctx, layers, l_c, ids, pos, target,
            eps_fracs, d, random_n=random_n, seed=prompt_seed + 9001)
        rb["task_gain_ratio_vs_random_mean"] = round(
            task_gain_pl / (rb["task_gain_random_mean"] + 1e-30), 4)
        rb["causal_ratio_vs_random_mean"] = round(
            max_p / (rb["causal_random_mean"] + 1e-30), 4)
        out.update(rb)
    return out
...
def run_real_static(base, revision, *, n_prompts=DEFAULT_REAL_PROMPTS,
                    ...
                    block_len=DEFAULT_BLOCK, eps_fracs=DEFAULT_EPS_FRACS,
                    task_family="induction", n_blocks=1, random_n=0,
                    dispatch=DISPATCH_PARTB, panel_path: Path | None = None,
                    alpha=1.0) -> dict:
    ...
    print(f"\n=== plastic instrument REAL static {spec} task={task_family} ===")
    ...
    for i in range(n_prompts):
        ids, pos, target = _make_prompt(
            task_family, ctx.tok, rng, block_len, n_blocks=n_blocks)
        try:
            rec = _analyze_prompt(
                ctx, layers, l_c, ids, pos, target,
                iters=iters, alpha=alpha, v0=None, eps_fracs=eps_fracs,
                random_n=random_n, prompt_seed=seed + i)
    ...
    if len(per_prompt) < 3:
        ...
        if random_n > 0:
            agg["task_gain_ratio_vs_random_mean"] = m("task_gain_ratio_vs_random_mean")
            agg["causal_ratio_vs_random_mean"] = m("causal_ratio_vs_random_mean")
    ...
    out = {
        ...
        "dispatch": dispatch,
        "task_family": task_family, "n_blocks": n_blocks,
        "params": {"n_prompts": n_prompts, "iters": iters, "k": k,
                   "block_len": block_len, "eps_fracs": list(eps_fracs),
                   "random_n": random_n, "alpha": alpha},
        "per_prompt": per_prompt, "verdict": verdict,
    }
    suffix = ""
    if task_family != "induction":
        suffix += f"_{task_family}"
    if n_blocks > 1:
        suffix += f"_k{n_blocks}"
    path = HERE / f"plastic_instrument_real_{safe_name(base)}{suffix}.json"
    path.write_text(json.dumps(out, indent=2))
    print(f"saved -> {path.name}")
    print(f"  verdict: {verdict.get('interpretation')}")
    _append_panel_row(panel_path, {
        "instrument": "plastic", "task_family": task_family, "n_blocks": n_blocks,
        "base": base, "revision": revision, **{k: verdict.get(k) for k in (
            "align_plastic_global_mean", "task_gain_ratio_mean", "causal_ratio_mean",
            "task_gain_ratio_vs_random_mean", "causal_ratio_vs_random_mean",
            "interpretation")},
    })
    _append_panel_row(panel_path, {
        "instrument": "global", "task_family": task_family, "n_blocks": n_blocks,
        "base": base, "revision": revision,
        "task_gain_mean": verdict.get("task_gain_global_mean"),
        "causal_ratio_vs_plastic": round(
            1.0 / (verdict.get("causal_ratio_mean") or 1e-30), 4)
            if verdict.get("causal_ratio_mean") else None,
    })
    return out
...
def run_real_track(base, revisions, *, n_prompts=DEFAULT_REAL_PROMPTS,
                   ...
                   ref_revision="step143000", task_family="induction",
                   dispatch=DISPATCH_PARTB) -> dict:
    ...
    rng = np.random.default_rng(seed)
    prompts = [_make_prompt(task_family, ctx_ref.tok, rng, block_len) for _ in range(n_prompts)]
    v_refs = []
    ...
    for rev in revisions:
        spec = f"{base}@{rev}"
        print(f"\n── track {spec} ({task_family}, alpha={alpha}) ──")
        ctx = load_context(spec)
        layers = ctx.layers
        l_c = _capacity_layer_index(len(layers))
        v_carry = None
        ...
        acc = _induction_accuracy(ctx, n_prompts, seed, block_len) if task_family == "induction" else None
        rec_ckpt = {
            "revision": rev, "step": _step_of(rev),
            "induction_acc": acc.get("induction_acc") if acc else None,
    ...
    suffix = f"_{task_family}" if task_family != "induction" else ""
    if alpha == 0.0:
        suffix += "_frozen"
    out = {
        ...
        "date": time.strftime("%Y-%m-%d"),
        "dispatch": dispatch,
        "task_family": task_family, "alpha": alpha,
        "params": {"n_prompts": n_prompts, "iters": iters, "alpha": alpha, "k": k},
        "trajectory": traj,
    }
    path = HERE / f"plastic_instrument_real_{safe_name(base)}{suffix}_track.json"
    path.write_text(json.dumps(out, indent=2))
    print(f"saved -> {path.name}")
    return out


def run_real_frozen_vs_track(base, revisions, *, task_family="recall", alpha=0.4,
                             **kwargs) -> dict:
    from icl_convergence_probe import safe_name

    kw = {k: v for k, v in kwargs.items() if k != "alpha"}
    frozen = run_real_track(
        base, revisions, task_family=task_family, alpha=0.0, **kw)
    tracked = run_real_track(
        base, revisions, task_family=task_family, alpha=alpha, **kw)
    out = {
        "mode": "frozen_vs_track", "probe": "plastic_instrument",
        "base": base, "task_family": task_family,
        "frozen": frozen, "tracked": tracked,
        "date": time.strftime("%Y-%m-%d"),
        "dispatch": kwargs.get("dispatch", DISPATCH_PARTB),
    }
    path = HERE / f"plastic_instrument_real_{safe_name(base)}_{task_family}_frozen_vs_track.json"
    path.write_text(json.dumps(out, indent=2))
    print(f"saved -> {path.name}")
    return out
...
def main():
    ...
    ap.add_argument("--random-n", type=int, default=0,
                    help="random-direction baseline draws (Part B: 20)")
    ap.add_argument("--task-family", default="induction",
                    choices=["induction", "multi_induction", "recall", "factual"])
    ap.add_argument("--n-blocks", type=int, default=1,
                    help="induction blocks for multi_induction (k>1)")
    ap.add_argument("--panel-out", default=None, help="append PANEL.jsonl rows")
    ap.add_argument("--dispatch-id", default=DISPATCH_PARTB)
    ap.add_argument("--frozen-vs-track", action="store_true",
                    help="recall checkpoint sweep: alpha=0 vs alpha=0.4")
    ap.add_argument("--alpha", type=float, default=0.4)
    args = ap.parse_args()

    panel_path = Path(args.panel_out) if args.panel_out else None

    if args.synthetic:
        run_synthetic(seed=args.seed)
        return
    if args.real:
        if args.frozen_vs_track:
            revs = args.revisions or ["step1000", "step2000", "step8000", "step143000"]
            run_real_frozen_vs_track(
                args.base, revs, n_prompts=args.n_prompts, iters=args.iters,
                k=args.k, seed=args.seed, block_len=args.block_len,
                task_family=args.task_family if args.task_family != "induction" else "recall",
                alpha=args.alpha, dispatch=args.dispatch_id)
            return
        if args.track:
            revs = args.revisions or ["step1000", "step2000", "step8000", "step143000"]
            run_real_track(args.base, revs, n_prompts=args.n_prompts,
                           iters=args.iters, k=args.k, seed=args.seed,
                           block_len=args.block_len, alpha=args.alpha,
                           task_family=args.task_family, dispatch=args.dispatch_id)
        else:
            run_real_static(args.base, args.revision, n_prompts=args.n_prompts,
                            iters=args.iters, k=args.k, seed=args.seed,
                            block_len=args.block_len, task_family=args.task_family,
                            n_blocks=args.n_blocks, random_n=args.random_n,
                            dispatch=args.dispatch_id, panel_path=panel_path,
                            alpha=args.alpha)
        return
