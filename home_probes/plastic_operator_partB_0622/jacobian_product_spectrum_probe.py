...
def transport_matvecs(ctx, layers, l_c, input_ids, pos):
    """Per-prompt transport T: capacity-layer repr -> final-norm readout repr.

    Returns (apply, apply_t, h_query, d) where apply(V)=T@V and apply_t(U)=T^T@U
    on numpy arrays (d x k). Shared by _operator_spectrum and plastic_instrument."""
    import torch
    from torch.autograd.functional import jvp, vjp

    cont, h_query = _transport_continuation(ctx, layers, l_c, input_ids, pos)
    d = int(h_query.shape[0])
    ...
    return apply, apply_t, h_query, d
...
def _operator_spectrum(ctx, layers, l_c, input_ids, pos, k, iters):
    ...
    apply, apply_t, _h_query, d = transport_matvecs(ctx, layers, l_c, input_ids, pos)
    sigma, Vr = topk_singular_values(apply, apply_t, d, k,
...
def run_with_context(ctx, n_samples=16, seed=42, tau=DEFAULT_TAU,
    ...
    for domain in DOMAINS:
        ...
        # full top-k operator/proxy spectra: enables the G5 Lyapunov SPECTRUM
        # (lambda_i = (1/depth) log sigma_op_i) and the dark/bright partition
        # (count of negative-lambda directions), not just lambda_max.
        # See OPERATOR_LIFT_SPEC.md G7 (F0-G5 bridge).
        agg["sigma_op_spectrum"] = [round(float(x), 5) for x in sigma_op_mean]
        agg["sigma_proxy_spectrum"] = [round(float(x), 5) for x in sigma_proxy[:k]]
